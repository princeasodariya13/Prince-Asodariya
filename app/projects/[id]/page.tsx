      ...
      <div className="relative z-10 w-full max-w-[1200px] mx-auto px-[clamp(1.5rem,5vw,5rem)] pt-8 pb-20">
        ...
        <header className="relative mb-10 md:mb-12">
          ...
          <div className="relative z-10 max-w-4xl">
            <Reveal>
              <div className="flex flex-wrap gap-2.5 mb-6 md:mb-8">
                {project.techStack.slice(0, 3).map((tech, i) => (
                  <span 
                    key={tech} 
                    className={`uppercase text-[0.6rem] md:text-[0.65rem] font-bold tracking-[0.15em] px-3 md:px-4 py-1.5 rounded-full border ${
            ...
            <Reveal delay={100}>
              <h1 className="font-display text-[clamp(1.85rem,6vw,4.5rem)] font-extrabold leading-[1.05] tracking-[-0.02em] text-[#111827] mb-5 md:mb-6">
                {project.title}
              </h1>
            </Reveal>
            
            <Reveal delay={200}>
              <p className="text-[0.95rem] md:text-[clamp(1.1rem,1.5vw,1.15rem)] text-[#4b5563] max-w-2xl mb-8 md:mb-12 leading-[1.7] font-medium tracking-tight">
                {project.description}
              </p>
            </Reveal>
              ...
              <div className="flex flex-wrap gap-4 items-center">
                {project.liveUrl && (
                  <a 
                    ...
                    className="group flex items-center gap-2.5 rounded-full bg-black px-6 md:px-7 py-3 md:py-3.5 text-sm md:text-base font-semibold !text-white transition-all duration-400 hover:scale-[1.04] hover:bg-gray-900 shadow-[0_8px_20px_rgb(0,0,0,0.16)] active:scale-95"
                  >
                    Live Deployment
                ...
                {project.githubUrl && (
                  <a 
                    ...
                    target="_blank" 
                    rel="noopener noreferrer" 
                    className="group flex items-center gap-2.5 rounded-full bg-white border border-[#e5e7eb] px-6 md:px-7 py-3 md:py-3.5 text-sm md:text-base font-semibold text-[#111827] transition-all duration-400 hover:scale-[1.04] hover:border-[#d1d5db] shadow-sm active:scale-95"
                  >
                    GitHub Repo
          ...
          {(project.purpose || project.impact) && (
            <Reveal>
              <section id="the-problem" className="grid grid-cols-1 md:grid-cols-[1fr_350px] gap-8 md:gap-12 items-start pt-4">
                {project.purpose && (
                  <div>
                    <div className="flex items-center gap-2.5 text-[#4f46e5] font-bold tracking-[0.2em] text-[0.65rem] uppercase mb-6 md:mb-8">
                    ...
                    <h2 className="font-display text-xl md:text-2xl lg:text-[1.8rem] font-bold tracking-tight text-[#111827] leading-[1.6] max-w-2xl">
                      {project.purpose}
                    </h2>
                  </div>
                )}
                {project.impact && (
                  <div className="md:pt-16">
                    <div className="text-[#9ca3af] font-bold tracking-[0.2em] text-[0.65rem] uppercase mb-4 md:mb-5">
                      Business Impact
                    </div>
                    <p className="text-[#4b5563] leading-[1.8] text-[0.9rem] md:text-[0.95rem] font-medium">
                      {project.impact}
                    </p>
                  </div>
                )}
              ...
              <section id="architecture" className="pt-8 md:pt-10 border-t border-[#f3f4f6]">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
                  {project.architecture.map((arch, i) => (
                    <div key={i} className="bg-white border border-[#f3f4f6] rounded-[24px] p-6 md:p-8 shadow-sm hover:shadow-md transition-shadow">
                      <h3 className="text-lg md:text-xl font-bold text-[#111827] mb-3 md:mb-4">{arch.title}</h3>
                      <p className="text-[#4b5563] text-[0.9rem] md:text-[0.95rem] leading-[1.7]">{arch.description}</p>
                    </div>
                  ))}
                </div>
              </section>
            ...
            <Reveal>
              <section id="technical-deep-dive" className="pt-8 md:pt-10 border-t border-[#f3f4f6]">
                ...
                <div className="grid md:grid-cols-3 gap-5 md:gap-6">
                  {/* Large Main Box */}
                  <div className="md:col-span-2 bg-[#f5f3ff] rounded-[24px] p-6 md:p-12">
                    <h3 className="text-xl md:text-3xl font-bold text-[#111827] mb-6 md:mb-8">{project.deepDive.title}</h3>
                    <div className="space-y-6 md:space-y-8">
                      <div>
                        <div className="text-[#8b5cf6] text-[0.65rem] font-bold tracking-[0.2em] uppercase mb-2">Core Architecture</div>
                        <div className="text-lg md:text-xl font-semibold text-[#1f2937]">{project.deepDive.architecture}</div>
                      </div>
                      <div>
                        <div className="text-[#8b5cf6] text-[0.65rem] font-bold tracking-[0.2em] uppercase mb-2">Training Methodology</div>
                        <div className="text-[0.9rem] md:text-[1rem] text-[#4b5563] leading-[1.7]">{project.deepDive.methodology}</div>
                      </div>
                    </div>
                  </div>
                  {/* Dataset Box */}
                  <div className="bg-white border border-[#f3f4f6] rounded-[24px] p-6 md:p-8 shadow-sm">
                    <h3 className="text-lg md:text-xl font-bold text-[#111827] mb-3 md:mb-4">Dataset</h3>
                    <p className="text-[#4b5563] text-[0.9rem] md:text-[0.95rem] leading-[1.7]">{project.deepDive.dataset}</p>
                  </div>
                  {/* Preprocessing Box */}
                  <div className="bg-white border border-[#f3f4f6] rounded-[24px] p-6 md:p-8 shadow-sm">
                    <h3 className="text-lg md:text-xl font-bold text-[#111827] mb-4 md:mb-6">Preprocessing Pipeline</h3>
                    <ul className="space-y-3 md:space-y-4">
                      {project.deepDive.pipeline.map((step, i) => (
                        <li key={i} className="flex gap-3 md:gap-4 items-start text-[#4b5563] text-[0.85rem] md:text-[0.95rem]">
                          <span className="text-[#d1d5db] font-bold font-mono text-xs md:text-sm">.0{i+1}</span>
                          {step}
                        </li>
                      ))}
                    </ul>
                  ...
                  {/* Metrics Box */}
                  <div className="md:col-span-2 bg-white border border-[#f3f4f6] rounded-[24px] p-6 md:p-8 shadow-sm flex flex-col justify-between">
                    <h3 className="text-lg md:text-xl font-bold text-[#111827] mb-6 md:mb-8">Evaluation Metrics</h3>
                    <div className="grid grid-cols-2 gap-6 md:gap-8">
                      {project.deepDive.metrics.map((metric, i) => (
                        <div key={i}>
                          <div className="text-4xl md:text-5xl font-black text-[#111827] mb-1 md:mb-2 font-display">
                            <AnimatedMetric value={metric.value} />
                          </div>
                          <div className="text-[#9ca3af] text-[0.6rem] md:text-[0.65rem] font-bold tracking-[0.2em] uppercase">{metric.label}</div>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <Reveal>
              <section id="results" className="pt-8 md:pt-10 border-t border-[#f3f4f6]">
                ...
                <div className="grid md:grid-cols-2 gap-10 md:gap-12 lg:gap-24 items-center">
                  <div>
                    <h2 className="font-display text-2xl md:text-[clamp(2rem,3.5vw,3rem)] font-bold text-[#111827] leading-[1.2] mb-5 md:mb-6">
                      {project.results.headline}
                    </h2>
                    <p className="text-[#6b7280] text-[0.95rem] md:text-[1.1rem] leading-[1.7]">
                      {project.results.description}
                    </p>
                  </div>
                  <div className="space-y-3 md:space-y-4">
                    {project.results.metrics.map((metric, i) => (
                      <div key={i} className="bg-[#ecfdf5] border border-[#d1fae5] rounded-[16px] p-5 md:p-6 text-[0.95rem] md:text-[1.1rem] font-bold text-[#111827] shadow-sm">
            ...
            <Reveal>
              <section id="learnings" className="pt-8 md:pt-10 border-t border-[#f3f4f6]">
                ...
                <div className="grid md:grid-cols-3 gap-5 md:gap-6 mb-16 md:mb-24">
                  <div className="bg-white border border-[#f3f4f6] rounded-[24px] p-6 md:p-8 shadow-sm">
                    <div className="text-[#9ca3af] text-[0.65rem] font-bold tracking-[0.2em] uppercase mb-3 md:mb-4">What I Learned</div>
                    <p className="text-[#4b5563] text-[0.9rem] md:text-[0.95rem] leading-[1.7]">{project.learnings.learned}</p>
                  </div>
                  <div className="bg-white border border-[#f3f4f6] rounded-[24px] p-6 md:p-8 shadow-sm">
                    <div className="text-[#9ca3af] text-[0.65rem] font-bold tracking-[0.2em] uppercase mb-3 md:mb-4">Trade-offs Made</div>
                    <p className="text-[#4b5563] text-[0.9rem] md:text-[0.95rem] leading-[1.7]">{project.learnings.tradeoffs}</p>
                  </div>
                  <div className="bg-white border border-[#f3f4f6] rounded-[24px] p-6 md:p-8 shadow-sm">
                    <div className="text-[#9ca3af] text-[0.65rem] font-bold tracking-[0.2em] uppercase mb-3 md:mb-4">Future Improvements</div>
                    <p className="text-[#4b5563] text-[0.9rem] md:text-[0.95rem] leading-[1.7]">{project.learnings.future}</p>
                  </div>
                </div>

                {/* Tech Stack Breakdown */}
                {project.techCategories && (
                  <div id="tech-stack" className="pt-10 md:pt-12 border-t border-[#f3f4f6]">
                    <div className="flex items-center gap-2.5 text-[#9ca3af] font-bold tracking-[0.2em] text-[0.65rem] uppercase mb-6 md:mb-8">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><ellipse cx="12" cy="5" rx="9" ry="3"></ellipse><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"></path><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"></path></svg>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-5 md:gap-6">
                      {Object.entries({
                        ...
                        <div key={category} className="bg-white border border-[#f3f4f6] rounded-[24px] p-6 shadow-sm">
                          <h4 className="text-md md:text-lg font-bold text-[#111827] mb-3 md:mb-4">{category}</h4>
                          <ul className="space-y-2.5 md:space-y-3">
                            {items.map(item => (
                              <li key={item} className="flex items-center gap-2 text-[#4b5563] text-[0.9rem] md:text-[0.95rem]">
